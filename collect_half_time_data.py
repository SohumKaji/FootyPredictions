...
for year in ['2016-2017']: # '2014-2015', '2015-2016', '2016-2017'
  ...
  for country, league in [['spain', 'laliga-']]:  
  #['england', 'premier-league-'], ['spain', 'laliga-'], ['germany', 'bundesliga-'], ['italy', 'serie-a-'], ['france', 'ligue-1-']
    ...
    for elem in reversed(year_elem):
      ...
      home_path = data_path+year[2:4]+'-'+year[7:]+country_dir+'/home/'+home_name+'.csv'
      away_path = data_path+year[2:4]+'-'+year[7:]+country_dir+'/away/'+away_name+'.csv'
      ...
      home_df = home_df.append(pd.DataFrame([home_stats_final], columns = collected_stats, index = [str(match_date)]))
      away_df = away_df.append(pd.DataFrame([away_stats_final], columns = collected_stats, index = [str(match_date)]))
            

      #Update the team data so far
      home_df.to_csv(home_path)
      away_df.to_csv(away_path)
      ...
      #temp_home_dict[(home_name,match_date)].append(home_stats(match_data, home_name, away_name, error_file))
      #temp_away_dict[(away_name,match_date)].append(away_stats(match_data, home_name, away_name, error_file))
