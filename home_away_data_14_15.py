...
def check_data(data, home_team, away_team, file, collected_stats, missing_val, total_errors):
    ...
    for elem in data:
        if((elem[0].isdigit())):
            elem_name = re.sub(r'(?<=[a-zA-Z])\s(?=[a-zA-Z])', '',elem).split(' ')[1].lower()
...
def home_stats(data, home_team, away_team, error_file):
    stat_list = []
...
def away_stats(data, home_team, away_team, error_file):
    stat_list = []
...
error_file = open('home_away_data_error_log_14_15.txt', 'w')
...
for country, ext in [['eng', '36'], ['ita','34'], ['esp','31'], ['ger', '8'], ['fra', '11']]:
 ...
 with open(country+'_hist_data_2014_15.pickle', 'wb') as f:
  pickle.dump([home_stats_2014_15, away_stats_2014_15], f)
